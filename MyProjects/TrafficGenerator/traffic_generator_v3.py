...
class TrafficGeneratorApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.running = False
        self.generator = None
        self.stats = {'sent': 0, 'errors': 0, 'bytes': 0, 'recv_bytes': 0, 'recv_packets': 0}
        self.history = {'time': [], 'pps': [], 'mbps': [], 'ingress_mbps': [], 'ingress_pps': []}
        self.start_time = 0
        self.protocol = 'UDP'
        self.initUI()
    ...
    def initUI(self):
        ...
        self.figure, (self.ax3, self.ax2, self.ax1) = plt.subplots(3, 1, figsize=(8, 8))
        self.canvas = FigureCanvas(self.figure)
        layout.addWidget(self.canvas)

        self.ax3.plot([], [], label='Ingress Mbps', color='green')
        ...
        initial_stats_text += "Packets Received: 0\n"
        initial_stats_text += "Errors: 0\n"
        initial_stats_text += "Total Bytes: 0\n"
        initial_stats_text += "Total Bytes Received: 0\n"
        initial_stats_text += "Average PPS: 0.00\n"
        initial_stats_text += "Average Mbps: 0.00\n"
        initial_stats_text += "Average Ingress Mbps: 0.00\n"
        initial_stats_text += "Average Ingress PPS: 0.00"
    ...
    def update_stats(self):
        if self.running:
            ...
            self.ax3.plot(self.history['time'], self.history['ingress_mbps'], label='Ingress Mbps', color='green')
            self.ax1.legend()
            self.ax2.legend()
            self.ax3.legend()
            self.canvas.draw()
            ...
            if total_time > 0:
                ...
                if self.stats['recv_bytes'] > 0:
                    ingress_mbps = (self.stats['recv_bytes'] / 1000000.0) / total_time
                    ingress_pps = (self.stats['recv_packets'] / total_time)

            text = f"Protocol: {self.protocol}\n"
            text += f"Server IP: {self.generator.real_server_address if hasattr(self.generator, 'real_server_address') and self.generator.real_server_address else 'N/A'}:{self.generator.real_server_port if hasattr(self.generator, 'real_server_port') and self.generator.real_server_port else 'N/A'}\n"
            ...
            text += f"Packets Received: {self.stats['recv_packets']}\n"
            text += f"Errors: {self.stats['errors']}\n"
            text += f"Total Bytes: {self.stats['bytes']}\n"
            text += f"Total Bytes Received: {self.stats['recv_bytes']}\n"
            text += f"Average PPS: {pps:.2f}\n"
            text += f"Average Mbps: {mbps:.2f}\n"
            text += f"Average Ingress Mbps: {ingress_mbps:.2f}\n"
            text += f"Average Ingress PPS: {ingress_pps:.2f}"
...
class TrafficGenerator:
    def __init__(self, config, stats, protocol):
        ...
        self.real_server_address = None
        self.real_server_port = None
        ...
        try:
            while self.running:
                ...
                if current_time > 0:
                    ...
                    if self.stats['recv_bytes'] > 0:
                        ingress_mbps = (self.stats['recv_bytes'] / 1000000.0) / current_time
                        ingress_pps = (self.stats['recv_packets'] / current_time)

                self.stats_update(current_time, pps, mbps, ingress_mbps, ingress_pps)
        except Exception as e:
    ...
    def send_packet(self):
        ...
        try:
            if self.protocol == 'TCP':
                ...
                self.real_server_address, self.real_server_port = sock.getpeername()  # get remote address and port
                ...
                while True:
                    ...
                    if not reply_packet:
                        break
                    self.stats['recv_bytes'] += len(reply_packet)
                    self.stats['recv_packets'] += 1
            else:  # UDP
    ...
    def stats_update(self, current_time, pps, mbps, ingress_mbps, ingress_pps):
        if isinstance(current_time, float) and isinstance(pps, (int, float)) and isinstance(mbps, (
        int, float)) and isinstance(ingress_mbps, (int, float)) and isinstance(ingress_pps, (int, float)):
            if current_time >= 0:
                window.history['time'].append(current_time)
                window.history['pps'].append(pps)
                window.history['mbps'].append(mbps)
                window.history['ingress_mbps'].append(ingress_mbps)
                window.history['ingress_pps'].append(ingress_pps)
