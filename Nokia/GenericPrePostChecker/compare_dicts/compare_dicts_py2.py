...
class CompareDicts:
    ...
    """Recoded for Python2 - please be sure that all updates made on compare_dicts.py are reflected here"""
    ...
    def _compare_dicts(self, pre, post, missing, new, changed, path=''):
        ...
        """
        for only_pre in set(pre.keys()) - set(post.keys()):
            missing.append("{} {} not found in post-dict".format(path, only_pre))

        for only_post in set(post.keys()) - set(pre.keys()):
            new.append("{} {} new in post-dict".format(path, only_post))

        path_stack = []
        for key in set(pre.keys()).intersection(set(post.keys())):
